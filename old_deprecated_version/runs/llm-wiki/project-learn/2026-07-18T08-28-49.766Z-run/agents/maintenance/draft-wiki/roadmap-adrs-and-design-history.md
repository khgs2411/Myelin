...
Repository branch, commit, and remote details are run evidence rather than durable project facts. For this maintenance run, the [sanitized checkout evidence](../repository-identity.json) reports an available `llm-wiki` checkout with an `origin` remote. The earlier draft's differing checkout snapshot is a stale run-evidence contradiction, not a durable fact. Likewise, any older documentation claim that this repository has no remote is stale and contradicted by the linked evidence; preserve that contradiction until the stale source is corrected rather than silently treating the no-remote claim as current.
