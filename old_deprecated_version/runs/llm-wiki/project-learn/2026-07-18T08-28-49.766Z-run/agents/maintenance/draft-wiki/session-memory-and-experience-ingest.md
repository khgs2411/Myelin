...
The start result's `queued_count` is the raw count of rows still in `experience_events`, and `selected_count` is derived from that count and the optional limit. It is not the count of immediately claimable rows: claimed tombstones retain their source event until terminal finalization, while leasing excludes those claimed rows through `countUnleasedExperienceEvents` and the lease selection path. Operators should therefore treat `queued_count` as queue occupancy, not a promise that every reported row can be claimed by the newly launched jobs.
...
The prompt tells the agent to create only low-risk trusted Session Memory directly. Ambiguous, risky, conflicting, or privacy-sensitive findings should become memory candidates instead. A verified implemented change to the target repository's architecture, public contract, operator workflow, persistence, or layout has an additional routing rule: it must produce a `scope: "project"` candidate, even when the worker also creates Session Memory. This prevents durable repository changes from being left only in short-lived Session Memory. Existing Session Memory reconciliation is limited to memories supplied in the reconciliation context.
...
Provider-created candidates are evidence-bearing leads, not free-form notes. Their evidence contains non-empty `observed_facts` plus `relevant_paths` and `uncertainties`; the proposed payload contains non-empty `durable_facts`, a non-empty `change_kind`, and `suggested_subjects` and `verification_needed`. `relevant_paths`, `uncertainties`, `suggested_subjects`, and `verification_needed` may be empty arrays where appropriate. Parser coverage in `tests/ingest/worker.test.ts` rejects a candidate that omits the required observed facts.
...
The implementation and status paths distinguish raw active events from unleased events, but current regression tests do not directly assert how manual-ingest `queued_count` should be presented when claimed rows remain. The documentation describes the current raw-row behavior; whether the CLI should instead expose a separate claimable count remains an operator-reporting decision.
