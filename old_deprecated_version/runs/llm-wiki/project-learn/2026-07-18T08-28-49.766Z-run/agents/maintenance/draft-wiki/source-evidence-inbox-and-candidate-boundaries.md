...
Ingest-provider candidates use a stricter lead contract than a bare summary. `src/ingest/worker.ts` requires `evidence.observed_facts` to contain at least one concrete observation and requires the `relevant_paths` and `uncertainties` arrays to be present, even when empty. `proposed_payload.durable_facts` likewise contains at least one durable claim; `change_kind` is non-empty; and `suggested_subjects` and `verification_needed` are present arrays that may be empty. The parser rejects candidates that violate those cardinalities, as covered by `tests/ingest/worker.test.ts`. This preserves the distinction between an evidence-bearing maintenance lead and a speculative implementation handoff.
