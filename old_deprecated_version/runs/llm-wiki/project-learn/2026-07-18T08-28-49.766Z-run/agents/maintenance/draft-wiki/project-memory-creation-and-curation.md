...
- chooses `create` mode when there is no curated Project Memory baseline, unless `--recreate` forces create mode;
...
Later ordinary `project learn` runs are maintenance-only. The mode decision is made from `project-memory.json` or bootstrap state in `src/project/project-memory-packet.ts` and `src/project/project-memory-curator-service.ts`. Both `curated` and `degraded` state retain a curated baseline, so a degraded latest maintenance result remains maintenance-eligible rather than forcing recreation. Status reports it as `curated_with_degraded_maintenance` and warns that the baseline is usable but the latest maintenance needs attention. `tests/project/project-memory-packet.test.ts`, `tests/project/project-memory-curator-service.test.ts`, and `tests/status/status-service.test.ts` cover this preservation rule. `myelin project learn <key> --recreate` is the explicit high-cost path that forces create mode again; it is separate from `myelin project reset <key> --clean --confirm <key>`, which deletes and reboots the project shell while preserving root `state/memory/memory.db`.
