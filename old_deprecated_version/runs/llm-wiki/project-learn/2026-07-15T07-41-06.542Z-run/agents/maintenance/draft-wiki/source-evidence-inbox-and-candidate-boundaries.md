...
- `projects/<key>/sources/inbox/*.json` is the V2 runtime durable-memory proposal inbox implemented by `src/inbox/runtime-inbox-items.ts`. These files are preserved source material, not canonical memory, and are the source for deterministic Project Memory candidate intake.
...
- Valid items are written as immutable pretty JSON under `projects/<key>/sources/inbox/<id>.json`.
- `projects/<key>/sources/index.md` and `projects/<key>/sources/inbox/index.md` are created lazily as source indexes.
...
The legacy gap inbox and the V2 runtime source inbox are both active concepts. Future agents should name the path they mean (`projects/<key>/inbox` versus `projects/<key>/sources/inbox`) and avoid using "inbox" as a generic synonym for candidates.
