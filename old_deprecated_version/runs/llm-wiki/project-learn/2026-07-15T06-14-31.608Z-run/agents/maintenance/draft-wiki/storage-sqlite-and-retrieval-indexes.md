...
The root database is `state/memory.db`, opened through `src/memory/db.ts`. `memoryDbPath(root)` resolves that path under the repository root, and `openMemoryDb(root)` creates `state/` as needed, configures Bun SQLite, opens the database, applies pragmas, and runs migrations. The connection setup uses:
...
Vector dimensions belong to each embedding provider contract rather than a global setting. Embedding contracts are persisted lifecycle state per retrieval scope: Session Memory and Project Memory each retain an active contract, resolve a desired contract from configuration, and report migration only when the two differ. Automatic selection is local-only and sticky: for an uninitialized scope it probes the local Ollama choices, then persists the selected contract; later processes reuse that active contract rather than silently changing providers. Gemini is an explicit configured provider, not an automatic fallback. Provider/model/dimension identities remain separate metadata contracts even when they share a vector-table width.

Changing a configured non-auto provider is a derived-index migration, not an in-place reinterpretation of old vectors. `myelin memory embeddings migrate` stages and indexes the desired contract before activation, `rollback` restores the retained previous contract, and `prune` is preview-first. Pruning refuses to remove historical state until every active canonical record or Project Memory section is indexed under the active contract. Status counts indexed, pending, and failed rows only for the active contract; historical rows remain diagnostic and do not by themselves make active retrieval unhealthy. These lifecycle operations and the active-contract health semantics live in `src/memory/embedding-contract-resolver.ts`, `src/memory/embedding-contract-lifecycle-service.ts`, and `src/status/embedding-retrieval-status.ts`.
