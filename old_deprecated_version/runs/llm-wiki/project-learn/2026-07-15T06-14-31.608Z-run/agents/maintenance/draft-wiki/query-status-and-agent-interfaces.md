...
- `--layer session|project`; omitted `--layer` uses Session Memory.
...
- `memory query` is not yet a complete multi-layer semantic router. The incomplete `--layer auto` facade was removed; callers must choose `session` or `project` when the default Session Memory path is not intended.
