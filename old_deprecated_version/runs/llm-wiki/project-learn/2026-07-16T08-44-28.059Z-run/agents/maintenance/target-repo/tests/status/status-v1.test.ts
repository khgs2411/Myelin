...
test("serializes the exact healthy V1 fixture", () => {
  expect(serializeStatusV1(normalized(healthy))).toEqual(healthy as unknown as ProjectOperationalStatusV1);
});

test("serializes the exact blocked V1 fixture", () => {
  expect(serializeStatusV1(normalized(blocked))).toEqual(blocked as unknown as ProjectOperationalStatusV1);
});
...
test("V1 owns exact top-level keys and excludes every shallow legacy field", () => {
  ...
  expect(Object.keys(output)).toEqual(["contract_version", "kind", "generated_at", "overall_state", "project", "installation", "session_memory", "project_memory", "warnings", "actions", "evidence"]);
...
function normalized(fixture: any): OperationalStatusResult {
  ...
  return result as OperationalStatusResult;
}
