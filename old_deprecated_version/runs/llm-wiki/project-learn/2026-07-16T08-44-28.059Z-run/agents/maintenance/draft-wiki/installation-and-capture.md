...
The supplied [repository-identity.json](../repository-identity.json) identifies this checkout's project key, branch, commit, and sanitized remote metadata. The inspected tests are fixture/service coverage rather than an end-to-end run against a live Codex hook installation and shell PATH configuration.
