...
The current path contract is `projects/<key>/` for canonical Markdown, `sources/<key>/` for preserved inputs, `state/<key>/` for project state, `runs/<key>/` for artifacts, and `state/memory/memory.db` for generated shared serving state. Legacy `projects/<key>/wiki`, `projects/<key>/sources`, and `state/memory.db` paths are migration inputs, not runtime aliases. Code or external consumers that reach those paths directly must update; the migration normalizes persisted references and fails closed rather than overwriting a destination collision (`src/runtime/fs.ts`, `src/runtime/layout.ts`, `tests/runtime/layout.test.ts`).
...
The supplied [repository-identity.json](../repository-identity.json) records this checkout as `llm-wiki` on `master` at `78cc13dfcc73145db780b80c38c7d247efd9eca9`, with an `origin` remote. It is deterministic checkout evidence, not a reachability or ownership assertion. Publication rewrites draft-local identity links to canonical `state/<key>/repository-identity.json` state (`tests/project/project-memory-draft-promotion.test.ts`).
