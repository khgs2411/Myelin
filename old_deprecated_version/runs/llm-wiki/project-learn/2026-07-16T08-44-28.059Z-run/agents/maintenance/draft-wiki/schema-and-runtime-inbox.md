...
The supplied [repository-identity.json](../repository-identity.json) is the deterministic checkout evidence for this run. The behavior evidence is strong for unit and command contracts, but this subject does not establish end-to-end provider-backed maintenance outcomes, automatic-maintenance scheduling/worker execution, or concurrent inbox-create/intake behavior.
