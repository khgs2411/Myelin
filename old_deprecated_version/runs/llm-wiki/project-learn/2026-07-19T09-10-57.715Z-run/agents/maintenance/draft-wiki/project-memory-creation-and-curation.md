...
Later ordinary `project learn` runs are maintenance-only. The mode decision is made from `project-memory.json` or bootstrap state in `src/project/project-memory-packet.ts` and `src/project/project-memory-curator-service.ts`. A `curated` or `degraded` Project Memory state both prove that a canonical baseline exists, so a degraded latest maintenance outcome remains maintenance-eligible rather than forcing a recreate. `myelin project learn <key> --recreate` is the explicit high-cost path that forces create mode again; it is separate from `myelin project reset <key> --clean --confirm <key>`, which deletes and reboots the project shell while preserving root `state/memory/memory.db`.
...
For evidence-bearing candidate leads, the maintenance contract requires at least one `evidence.observed_facts` entry and at least one `proposed_payload.durable_facts` entry. `relevant_paths`, `uncertainties`, `suggested_subjects`, and `verification_needed` remain required arrays but may be empty. These fields describe a lead, not an accepted wiki claim: maintenance verifies the current repository, tests, and existing pages before selecting a disposition or a target page.
...
`myelin memory maintain project <key> --review` creates a validated maintenance review and stops before publication. An operator can later publish that exact review with `myelin memory maintain project <key> --promote <run>`. Promotion cannot be combined with review, dry-run, recreate, or resume modes, and it does not invoke the maintenance authoring agent again.

`src/project/project-memory-review-checkpoint.ts` records hashes for the review artifacts and draft markdown, the canonical Project Memory file set and bytes, the target-repository snapshot, the input packet, and the reviewed pending-source set. Before promotion, Myelin rejects a source that is not an unpromoted validated maintenance review, a changed reviewed artifact, a changed canonical baseline, a changed repository snapshot, or a reviewed source that is no longer pending or whose packet record changed. It then copies the verified draft into a new apply run and uses the normal journal-backed promotion path. The resulting state records `curation_kind: "human_reviewed"` and `reviewed_from_run`.

`src/commands/memory.ts` emits foreground progress for non-JSON maintenance commands from command acceptance through preflight and final run status; the maintenance service also reports agent start, bounded retry, and completion. This is operational visibility only: progress does not relax the review, evidence, or publication gates.
...
- `tests/project/project-memory-curator-service.test.ts` for create-plus-maintenance composition, maintenance-only candidate processing, degraded-baseline maintenance eligibility, review mode, exact reviewed-draft promotion, and v2 state fields.
- `tests/project/project-memory-agent-maintenance-service.test.ts` and `tests/commands/memory.test.ts` for the maintenance prompt/report contract, bounded retry, and foreground progress events.
