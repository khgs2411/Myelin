...
Provider-created durable Project Memory candidates are evidence-bearing leads. Their current contract requires non-empty `evidence.observed_facts` and `proposed_payload.durable_facts`; `evidence.relevant_paths`, `evidence.uncertainties`, `proposed_payload.suggested_subjects`, and `proposed_payload.verification_needed` are always arrays, including when empty. This preserves enough context to verify a durable claim without letting the ingest worker choose canonical page placement. Legacy candidates and runtime-inbox candidates can have different stored shapes, so the maintenance pass treats their metadata as routing context and verifies concrete claims against the current repository before writing documentation.
