...
`IngestService.start` reports `queued_count` from `countExperienceEvents`, which counts every active `experience_events` row. Claimability is narrower: `countUnleasedExperienceEvents` excludes rows with a `claimed` tombstone, while a worker claims only still-claimable rows. Consequently the manual ingest count can include work already leased by another job and overstate immediately claimable work. This is the current implementation behavior, not a settled operator-accounting policy; the inspected tests cover ingest start and leasing but do not establish whether the user-facing count should instead be claimable-only.
...
When leased evidence describes an implemented durable change to this repository's architecture, public contract, operator workflow, persistence, or layout, the worker must emit a `scope: "project"` candidate even if it also writes Session Memory. It must not leave that change only in Session Memory. The candidate's `evidence` contains at least one observed fact, relevant paths, and uncertainties; its `proposed_payload` contains at least one durable fact, a change kind, suggested subjects, and verification needed. Project maintenance—not the ingest worker—then verifies the lead against repository evidence and selects the canonical page. `tests/ingest/worker.test.ts` covers this prompt contract and the scheduling of Project Memory maintenance after a project candidate is created.
