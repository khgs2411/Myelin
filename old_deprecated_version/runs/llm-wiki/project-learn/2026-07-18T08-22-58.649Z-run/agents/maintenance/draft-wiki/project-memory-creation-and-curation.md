...
A `project-memory.json` status of `degraded` still represents an existing curated baseline, so packet construction continues in maintenance mode rather than recreating Project Memory. After a later maintenance pass, canonical state returns to `curated`; the latest maintenance outcome remains independently `degraded` and its reasons stay visible. Status reports this combination as `curated_with_degraded_maintenance`, with a maintenance-degraded warning rather than `PROJECT_CURATION_NOT_READY`. `tests/project/project-memory-packet.test.ts`, `tests/project/project-memory-curator-service.test.ts`, and `tests/status/status-service.test.ts` cover the mode, state, and status behavior.
