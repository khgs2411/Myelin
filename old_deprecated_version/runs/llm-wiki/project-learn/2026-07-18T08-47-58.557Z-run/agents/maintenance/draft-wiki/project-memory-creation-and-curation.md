...
- chooses `create` mode when there is no `curated` or `degraded` Project Memory baseline, unless `--recreate` forces create mode;
- runs create mode, then immediately runs maintenance mode against pending project candidates and handoffs before promotion.

A previous curated baseline remains maintenance-eligible when its latest Project Memory state is `degraded`. `hasCuratedProjectMemoryBaseline` in `src/project/project-memory-state.ts` accepts both `curated` and `degraded`, so a maintenance failure or degraded result does not silently force a new create-mode pass over existing canonical pages. `tests/project/project-memory-curator-service.test.ts` and `tests/project/project-memory-packet.test.ts` cover the degraded-baseline behavior.
