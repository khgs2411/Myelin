...
**Run-evidence contradiction:** the preceding commit snapshot is preserved from an earlier maintenance run. The current run's sanitized `repository-identity.json` reports a different head commit while confirming the same branch and `origin`; checkout identity remains run evidence, not a durable Project Memory fact.
