...
The start result's `queued_count` is the raw `experience_events` count for the project. It is therefore an operator-facing queue-size snapshot, not a claimability estimate: rows that have already acquired a `claimed` tombstone can still be included until finalization deletes their source rows. `countUnleasedExperienceEvents` is the separate measure for rows not currently claimed.
...
Provider-produced candidates are evidence-bearing leads. The ingest-output parser requires an `evidence` object containing non-empty `observed_facts` plus `relevant_paths` and `uncertainties`, and a `proposed_payload` containing non-empty `durable_facts` plus `change_kind`, `suggested_subjects`, and `verification_needed`. The four list fields other than the two fact lists may be empty. Every candidate also needs source tombstone references. This is distinct from a runtime-inbox candidate, whose evidence and payload preserve the inbox item's source metadata rather than claiming provider-observed implementation facts.

When leased evidence establishes an implemented, durable change to this repository's architecture, public contract, operator workflow, persistence, or layout, the worker must emit a `scope: "project"` candidate even if it also writes Session Memory. A project candidate is a maintenance lead, not a direct wiki write; it is the mechanism that prevents verified durable changes from being represented only as Session Memory.
