...
Provider-produced ingest candidates use a stricter evidence-bearing contract than deterministic runtime-inbox normalization. Their parser requires at least one observed fact and at least one proposed durable fact; it also requires the `relevant_paths`, `uncertainties`, `suggested_subjects`, and `verification_needed` arrays, although those four arrays may be empty. For a verified implemented repository change affecting architecture, a public contract, an operator workflow, persistence, or layout, the ingest worker must create a project-scoped candidate rather than leave the change only in Session Memory. Project Memory maintenance still owns verification, page selection, and the terminal disposition.
