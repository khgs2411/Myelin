...
Provider-created candidates have an evidence-bearing contract in `src/ingest/worker.ts`. A verified implemented durable change to the current repository's architecture, public contract, operator workflow, persistence, or layout must produce a `scope: "project"` candidate rather than Session Memory alone. Its `evidence.observed_facts` and `proposed_payload.durable_facts` must each contain at least one item; `relevant_paths`, `uncertainties`, `suggested_subjects`, and `verification_needed` are required arrays and may be empty. Maintenance, not ingest, chooses whether and where to update canonical markdown after repository verification.
