...
The command's queued count is the raw `experience_events` count from `countExperienceEvents`, so it includes rows that may already have tombstone-backed leases. Claimability is a separate concept: `countUnleasedExperienceEvents` excludes active rows with claimed tombstones. Operators should therefore read the start count as queued raw rows, not as a guarantee that the same number of rows can be leased by a new worker. Current code establishes that distinction but does not declare whether the user-facing count is an intentional reporting policy.
