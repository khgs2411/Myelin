...
- chooses `create` mode when no curated baseline exists, unless `--recreate` forces create mode;
...
A `degraded` `project-memory.json` still represents an existing curated baseline and selects maintenance mode. This prevents a failed or incomplete later maintenance pass from discarding usable canonical markdown or forcing recreation. A subsequent successful maintenance pass restores the top-level state to `curated`; its maintenance record retains the latest maintenance outcome and any degradation details.
