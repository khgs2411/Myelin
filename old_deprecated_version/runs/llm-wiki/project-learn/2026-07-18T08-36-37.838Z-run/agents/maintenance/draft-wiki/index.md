...
- The repository has many historical design files under `docs/archive/` and `docs/design/`; active canonical docs, current code, and regression tests take precedence when they conflict.
- Some roadmap content is more current than older alignment text; subject pages preserve relevant conflicts rather than treating older planning prose as implementation proof.
