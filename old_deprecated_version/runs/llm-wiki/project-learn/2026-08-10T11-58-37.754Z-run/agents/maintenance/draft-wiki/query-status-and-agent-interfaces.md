...
`src/memory/session-memory-query.ts` is the Session Memory query service. It requires an available sqlite-vec vector table, indexed active Session Memory embeddings, and an embedding for the normalized question. It searches vectors, hydrates rows from `session_memories`, filters out non-active rows by default, filters by branch context when requested, and returns source tools `query-embedding-cache` and `session-memory-vector-index`. Semantic vector order remains authoritative for every question, including wording such as “latest,” “recent work,” or “last session”; the service does not apply a timestamp-based recency rerank. It initially fetches a bounded candidate window, then expands to all searchable vectors when lifecycle or branch filtering leaves fewer than the requested number of matches. This prevents inactive vectors from consuming the whole initial window and starving active results. Tests in `tests/memory/session-memory-query.test.ts` verify explicit degradation when sqlite-vec is unavailable, cache reuse for normalized questions, active-only lifecycle filtering, branch-context filtering, semantic-first recency questions, and inactive-window expansion.

`src/query/project-memory-query-service.ts` is the Project Memory retrieval service used by `--layer project`. It searches derived Project Memory retrieval vectors, then resolves hits back to canonical markdown sections under `projects/<key>/`. It returns inline section content only when the current markdown section hash matches the indexed row and the section body is under the inline-size threshold. If the section is too large, stale, or missing, the result is a canonical reference with `reference_reason` rather than stale inline content. Reranking starts from reciprocal-rank fusion when both vector and FTS recall exist (or vector distance otherwise), then adds exact title/path/body coverage signals and navigation penalties. It does not force a weak `and`-facet match above stronger compound-query evidence. Tests in `tests/query/project-memory-query-service.test.ts` cover markdown hydration, section ids, size-threshold fallback, stale-hash fallback, missing-markdown fallback, exact-section reranking, and the no-forced-facet rule.
...
- Status citations name the project state files used: `state/<key>/project.json`, `freshness.json`, and `update-state.json`.

`confidence` is a deterministic retrieval-evidence score, not a probability and not `1 - cosine distance`. Session Memory confidence uses non-degraded retrieval, source-backed evidence, result coverage, and relative separation within the returned set. Project Memory confidence uses canonical inline content, agreement between vector and FTS recall, and exact section rerank signals. Empty results return `0`, and degradation either returns `0` or caps confidence when usable canonical inline content remains. The answer text is a deterministic rendering of matches or degradation reason, not a generated synthesis.
...
Current status payloads add an optional `briefing.session_continuity` object with contract version `myelin.session_continuity.v1`; consumers of older status payloads must tolerate the whole `briefing` object being absent. The human renderer places the same briefing immediately after the project identity.

Continuity is selected by durable `ingest_jobs.id`, never by an ingest worker's internal prompt sub-batches. Active memories first have to pass every structural eligibility gate: they need an existing same-project ingest job, a nonempty source-reference list, and same-job tombstones that are finalized as `output`, have parseable metadata, and back-reference the memory. A memory with only `session.start` provenance is excluded; a memory with both control-plane and content provenance remains eligible but is degraded. The exclusion reasons are `missing_ingest_job_id`, `missing_ingest_job`, `missing_source_reference`, `missing_tombstone`, `foreign_project_tombstone`, `cross_job_tombstone`, `non_output_tombstone`, `missing_output_backreference`, `malformed_source_metadata`, and `control_only_provenance`.

Among eligible memories, the newest ingest-job group is `anchor_job`. `continuity`, `verification`, and `decision` memory select the latest eligible group for the `current_state`, `completed_outcomes`, and `recent_decisions` channels respectively; active `blocker` and `next_action` memories are included across every eligible group. Each item identifies its relation as `anchor_job` or `prior_job`, and its provenance state as `content_only` or `mixed_control_content`.

The observable state and gate precedence are:

- `unavailable` when no eligible anchor job exists.
- `degraded` when an eligible result has mixed provenance or any structural exclusion.
- `lagging` when integrity is valid but unleased content events, leased content events, or running ingest jobs make freshness lagging.
- `ready` only when an eligible result has valid integrity and current freshness.

This is strict precedence: `unavailable`, then `degraded`, then `lagging`, then `ready`. `integrity.state` is either `valid` or `degraded`; it describes provenance integrity, not whether the memory's statements are semantically true. Freshness counts only content events, so `session.start` does not by itself make the briefing lagging. The corresponding reason codes are `no_eligible_anchor_job`, `content_events_unleased`, `content_events_leased`, `ingest_running`, `mixed_control_content_provenance`, `newer_ineligible_ingest_job`, and `channel_memory_excluded`. `src/memory/session-current-continuity.ts` implements these gates and `tests/memory/session-current-continuity.test.ts` covers the selection, mixed-provenance, freshness, and fail-closed cases.

The status summary includes:

- project key, display name, and project directory from `state/<key>/project.json`;
- latest session pointer from `projects/<key>/sessions/*.md` by file mtime;
- stale status, changed paths, impacted pages, and update time from `state/<key>/freshness.json`;
- latest run and completed stage from `update-state.json`, falling back to `bootstrap-state.json` or the newest run directory.

Project Memory curation status distinguishes a missing or unusable baseline from a degraded latest maintenance result. When `project-memory.json` still has a curated baseline and readable canonical markdown, a `degraded` or failed latest maintenance record yields `curated_with_degraded_maintenance`: the baseline remains usable, status is attention, and maintenance—not recreate mode—is the recovery path. Missing canonical markdown or an invalid/non-curated state remains blocking. `src/status/project-memory-inspector.ts` implements this precedence and `tests/status/status-service.test.ts` covers the degraded-baseline lifecycle.
...
The status facade is still incomplete outside this additive continuity contract: latest-session lookup continues to use `wiki/sessions/*.md` modification time for its legacy pointer. `docs/IMPLEMENTATION_ALIGNMENT.md` identifies that limitation separately from the continuity briefing.
