...
Provider output is parsed and validated before any output is applied. The subsequent writes, lifecycle transitions, output references, and tombstone finalization run in one database transaction, so an invalid output cannot leave a partial Session Memory apply.

## Control-Plane Event Boundary And Repair

`session.start` is the currently supported Session Memory control-plane event kind. It is a scheduling signal, not curator evidence: a batch containing only that event finalizes its tombstone as `no_output` with terminal decision `control_event` without invoking the provider, while a mixed batch excludes its control-plane rows from the provider evidence. It can still force the bounded auto-maintenance drain described below.

`myelin memory session repair <project-key> [--apply] [--json]` corrects pre-existing active records under the versioned `session-control-events-v1` policy. Preview is read-only. It proposes a retraction only when all of these gates pass, in order:

1. the record is still `active` for the requested project;
2. it has at least one cited tombstone and every cited tombstone can be resolved to an event kind;
3. every resolved kind is the control-plane `session.start` value; and
4. on `--apply`, the cited-source hash still matches the preview candidate.

Missing provenance, unresolved event kinds, or any mixed content/control provenance fail closed and are not automatically retracted. An apply writes a prepared then completed repair journal under `runs/<project-key>/memory-session-repair/<run-id>/report.json`, transitions qualifying records from `active` to `retracted` with a stable lifecycle reason, and retains the Session Memory row, tombstones, contexts, and provenance. A later preview is therefore idempotent: it does not propose an already retracted record.
...
`next_action` is deliberately short-lived. When supplied evidence shows an active action is complete, superseded by a new next step, or obsolete, ingest curation must retract it or replace it rather than leave it as a noop. Before creating a new action, the curator checks inspectable current repository and machine state so it does not insert an already-completed instruction. The active reconciliation context deliberately includes current next actions for that lifecycle review.
...
`refreshDetachedIngestJobStatus` marks a running job failed if its recorded PID is no longer alive. Recoverable claimed tombstones are selected before new rows on the next lease attempt, and their metadata records the prior attempt. Before `IngestService.start()` counts queued rows or creates jobs, it reconciles terminally tombstoned replay rows and reports the resulting `reconciled_count`; a replay-only queue therefore returns `no_work` without launching a worker. The human CLI also prints a reconciliation notice when that count is nonzero.
...
Scheduled workers run `bun src/maintenance/worker.ts <project-key>` detached, write logs under `runs/<project-key>/logs/<run-id>.log`, and store state in `state/<project-key>/auto-memory-maintenance.json`. The worker runs `IngestService.start`, waits for ingest drain with configured polling and timeout, then indexes pending Session Memory embeddings through `SessionMemoryIndexService`. Its state and result include `reconciled_count` alongside queued, indexing, and remaining-queue counts. If a started ingest leaves the queue at or above its pre-run size, the worker records a failed no-progress result with `rescheduled: false` and does not create a continuation worker; it only reschedules after progress when queued work still meets the threshold or embeddings remain pending. Tests in `tests/maintenance/auto-memory-maintenance.test.ts` cover disabled mode, threshold gating, forced SessionStart drain, lock behavior, dead-lock recovery, reconciliation reporting, no-progress termination, and completed state recording.
