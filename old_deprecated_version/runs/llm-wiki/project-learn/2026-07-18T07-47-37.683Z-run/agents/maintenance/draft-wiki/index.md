...
- Historical design files under `docs/archive/` and `docs/design/` explain prior intent but are not implementation authority; current code, active docs, and regression tests control these pages.
- Older alignment and roadmap material can conflict. The subject pages preserve material contradictions rather than silently treating historical prose as current behavior.
