...
### Durable-change routing and candidate lead contract

Ingest must not reduce a verified, implemented repository change to Session Memory alone. The worker prompt in `src/ingest/worker.ts` requires a `scope: "project"` candidate when leased evidence establishes a durable architecture, public-contract, operator-workflow, persistence, or layout change. A continuity or verification Session Memory may accompany that lead, but it does not replace it. `tests/ingest/worker.test.ts` asserts that prompt contract and that created project candidates schedule downstream Project Memory maintenance.

Provider-created candidate statuses are only `pending` and `needs_review`; both still require curator verification. A candidate is accepted only when it supplies the routing fields plus evidence and a proposed payload. Its evidence must contain non-empty `observed_facts`, `relevant_paths`, and `uncertainties`; its proposed payload must contain non-empty `durable_facts`, `change_kind`, `suggested_subjects`, and `verification_needed`. The parser regression test rejects a candidate with an empty evidence object. These are leads for maintenance, not page-selection instructions: the maintenance curator verifies the change against the repository and selects the canonical page.
