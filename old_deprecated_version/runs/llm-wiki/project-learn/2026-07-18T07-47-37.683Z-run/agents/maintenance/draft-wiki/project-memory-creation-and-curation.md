...
Repository checkout identity is deterministic run evidence, not a substitute for repository verification. A draft page may cite [repository-identity.json](repository-identity.json) when that artifact is relevant. During publication, `src/project/project-memory-draft-promotion.ts` rewrites that run-local target to canonical project state (`state/<key>/repository-identity.json`) and rejects the link if canonical identity is unavailable. This implementation behavior conflicts with an older handoff that proposed rejecting every identity link; the current code and its promotion contract are authoritative for the active workflow.
...
## Curated baseline versus latest maintenance

Project Memory state distinguishes a usable canonical baseline from the outcome of the latest maintenance run. `src/project/project-memory-state.ts` considers both `status: "curated"` and `status: "degraded"` to retain a curated baseline. As long as canonical markdown is readable, packet construction and curator mode remain in maintenance mode rather than falling back to create mode.

The status inspector applies the outcome precedence: invalid state or missing canonical markdown blocks curation; otherwise a top-level degraded status, or nested `maintenance.status` of `degraded` or `failed`, produces `curated_with_degraded_maintenance` with an attention warning. The baseline remains usable, while the latest maintenance issue remains visible. `tests/status/status-service.test.ts` covers a legacy degraded state and verifies that it does not report `PROJECT_CURATION_NOT_READY`.
