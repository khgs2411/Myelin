...
Provider-created candidates have a stricter lead contract than legacy stored rows. `parseIngestWorkerOutput` in `src/ingest/worker.ts` requires `evidence.observed_facts` to contain at least one non-empty fact, while `relevant_paths` and `uncertainties` are required arrays that may be empty. It likewise requires `proposed_payload.durable_facts` to contain at least one non-empty fact, with required (possibly empty) `suggested_subjects` and `verification_needed` arrays plus a non-empty `change_kind`. The worker rejects malformed output before it is applied. This makes a candidate evidence-bearing without letting it assert canonical truth: Project Memory maintenance still verifies the lead against current repository evidence and records a disposition.

Ingest-created project candidates are a distinct producer path. After a worker applies project-scoped candidates, `runIngestWorker` schedules optional Project Memory maintenance with the `session_memory_candidate_created` reason; a scheduler failure does not roll back the durable Session Memory/lead output. That route complements, rather than bypasses, runtime-inbox intake and the curator's verification boundary.
