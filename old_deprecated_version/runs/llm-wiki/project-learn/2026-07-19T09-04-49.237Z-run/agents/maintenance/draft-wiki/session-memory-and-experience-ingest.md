...
The start result's `queued_count` and derived `selected_count` are a raw `experience_events` snapshot taken before worker leases are claimed; they are not a guarantee that the same number remains claimable when workers run. `src/ingest/worker.ts` separately filters output references to the tombstones claimable by that worker. This distinction is observable during overlapping starts, but the current implementation does not expose a separate claimable-count reporting field. Normal auto-maintenance prevents duplicate scheduling with running-job and project-lock checks; `SessionStart` remains the deliberate bounded-drain exception described below.
