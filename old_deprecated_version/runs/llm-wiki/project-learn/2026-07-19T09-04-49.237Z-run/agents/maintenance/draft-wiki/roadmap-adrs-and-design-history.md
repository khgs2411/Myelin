...
This historical draft also contained checkout-specific branch, commit, and remote assertions. They conflict with the sanitized checkout evidence for the current maintenance run and are not durable Project Memory facts. Preserve that contradiction as run evidence rather than treating either checkout snapshot, including any old no-remote claim, as canonical repository prose.
