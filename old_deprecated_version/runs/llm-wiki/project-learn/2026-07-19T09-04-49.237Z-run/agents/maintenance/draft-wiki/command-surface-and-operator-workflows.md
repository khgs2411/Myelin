...
bun src/cli.ts memory maintain project class-kit --review
bun src/cli.ts memory maintain project class-kit --promote runs/class-kit/project-learn/<review-run>
bun src/cli.ts memory review class-kit --status insufficient_evidence --json
```

Review promotion verifies the reviewed packet, pending sources, draft/report artifacts, canonical wiki baseline, and target-repository fingerprint before journaled publication. It fails rather than silently rerunning authoring or applying a draft after any of those inputs drift. Human command mode emits foreground stage progress and heartbeats; `--json` keeps stdout machine-readable.
