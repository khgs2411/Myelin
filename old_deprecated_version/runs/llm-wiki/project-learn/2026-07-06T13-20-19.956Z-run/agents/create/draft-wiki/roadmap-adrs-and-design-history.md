...
As of this snapshot, Roadmap Steps 0 through 6 are marked complete or retired where appropriate. The active item is Step 6.5, `Define the vision-quality first-create gate`. The roadmap says the current first-create foundation has useful mechanics, but Project Memory still needs a product-quality gate driven by representative questions from `MY_VISION.md`, real provider dogfood, precise citations, and explicit failure wording when docs are only foundation-valid. Steps 7 through 11 remain `open`: maintenance and candidate promotion, Project Memory query and CLI contract, CLI dogfood acceptance, MCP wrapping, and later Practice/Personal Memory roadmap expansion are intentionally behind the first-create quality problem.
