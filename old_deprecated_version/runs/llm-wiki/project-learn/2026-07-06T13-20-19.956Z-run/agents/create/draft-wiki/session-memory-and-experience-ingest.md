...
from the target repo cwd, with `MYELIN_ROOT`, `MYELIN_INGEST_JOB_ID`, `MYELIN_INGEST_PROJECT`, and `MYELIN_CAPTURE_DISABLED=1` in the environment. Logs go to `projects/<project-key>/logs/ingest-<job-id>.log`. If launch fails, the job is marked `failed` with `error_json.code = "detached_worker_launch_failed"`.
