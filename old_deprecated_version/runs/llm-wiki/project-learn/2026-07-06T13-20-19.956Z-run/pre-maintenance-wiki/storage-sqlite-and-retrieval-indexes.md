...
Embedding behavior is controlled by config and environment. `EMBEDDING_STUB_RESPONSES_DIR` supports deterministic tests, while `GOOGLE_API_KEY` or `GEMINI_API_KEY` is required for the Gemini embedding provider. `EMBEDDING_BATCH_SIZE` controls default provider batching, with command-level `--batch-size` capped by config constants.
