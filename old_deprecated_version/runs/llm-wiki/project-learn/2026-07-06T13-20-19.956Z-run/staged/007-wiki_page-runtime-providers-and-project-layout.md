...
- Embedding settings for the current Gemini embedding provider: model, dimensions, batch size, and optional stub response directory.
...
`myelin.config` in this snapshot sets `DEFAULT_PROVIDER=codex`, uses separate pipeline/query/ingest Codex models and reasoning efforts, defines Claude model profiles, enables auto memory maintenance, and leaves embedding provider credentials as local `.env` or environment concerns. Tests in `tests/runtime/runtime.test.ts` verify precedence, numeric bounds, default embedding contracts, ingest limits, and auto-maintenance defaults.
...
- The LLM provider abstraction is only wired for Codex and Claude. Gemini appears only as the embedding provider in current runtime config.
