...
Provider-created candidates have a stricter evidence-bearing lead contract. `src/ingest/worker-output.schema.json` and `parseIngestWorkerOutput` in `src/ingest/worker.ts` require every candidate to include `evidence.observed_facts`, `evidence.relevant_paths`, and `evidence.uncertainties`, plus `proposed_payload.durable_facts`, `change_kind`, `suggested_subjects`, and `verification_needed`. The two fact arrays must each contain at least one item; the remaining arrays may be empty. `tests/ingest/worker.test.ts` rejects a candidate without observed facts and verifies that the ingest prompt directs verified durable repository changes to a `scope: "project"` candidate rather than Session Memory alone. After a project candidate is created, ingest schedules Project Memory maintenance; that scheduling is a routing step, not evidence that the candidate has been accepted into curated markdown.
