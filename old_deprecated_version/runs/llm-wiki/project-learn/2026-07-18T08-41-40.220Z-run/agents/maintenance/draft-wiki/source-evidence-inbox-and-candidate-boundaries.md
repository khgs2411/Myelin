...
Myelin keeps source material separate from synthesized memory. The repository contract in `AGENTS.md` defines the active layers as `repo/`, preserved source evidence under `sources/`, curated markdown under `projects/<key>/`, and generated `state/`. New source material should be classified, preserved, reconciled into the smallest canonical documentation surface, and left with terminal status instead of being silently discarded.
...
Provider-created project candidates have a stricter evidence contract than legacy leads: `src/ingest/worker-output.schema.json` requires non-empty `observed_facts` and `durable_facts`, while `relevant_paths`, `uncertainties`, `suggested_subjects`, and `verification_needed` are always-present arrays that may be empty. `src/ingest/worker.ts` directs workers to emit these candidates for verified durable repository changes rather than representing the change only as Session Memory. Project Memory maintenance remains responsible for repository verification, page selection, and the final disposition.
...
The active layout is `projects/<key>/` for canonical markdown, `state/<key>/` for per-project state, `sources/<key>/` for preserved evidence, `runs/<key>/` for artifacts, and `state/memory/memory.db` for shared generated SQLite state. `src/runtime/layout.ts` rewrites recorded legacy `projects/<key>/wiki/` and `state/memory.db` values during migration; those names are compatibility inputs, not current output locations.
