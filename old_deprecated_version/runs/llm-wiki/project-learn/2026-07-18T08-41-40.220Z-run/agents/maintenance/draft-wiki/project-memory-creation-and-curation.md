...
A degraded maintenance result is not by itself evidence that the curated baseline disappeared. A legacy state with `status: "degraded"` remains eligible for maintenance; after a successful maintenance pass, the top-level state returns to `curated` while `maintenance.status` and `maintenance.degraded_reasons` preserve the latest maintenance limitation. `tests/project/project-memory-curator-service.test.ts` covers this baseline-preservation behavior.
