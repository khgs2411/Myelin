...
An earlier draft recorded a checkout snapshot for `llm-wiki` on `master` at `afa43fe26e113ee4f0195eb3c474a077a2b1b17e` with an `origin` remote. Current sanitized checkout evidence reports a different commit and also an `origin` remote. These branch, commit, and remote values are run evidence rather than durable Project Memory facts; the conflicting snapshots are preserved here as a documentation contradiction, and neither snapshot is canonical identity prose. A claim that this checkout has no remote would likewise conflict with both observed snapshots.
