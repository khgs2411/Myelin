...
Experience Log ingest can also create project candidates directly. Its stricter worker contract in `src/ingest/worker-output.schema.json` requires observed facts, relevant paths, uncertainties, durable facts, a change kind, suggested subjects, and verification needs. `src/ingest/worker.ts` directs the ingest agent to emit a project candidate—not only Session Memory—when leased evidence establishes an implemented durable repository change. The maintenance agent, not the ingest agent, still selects the canonical page after checking the candidate against the repository and existing wiki.
...
`deferred_unsafe_change` and `blocked_by_runner_failure` instead preserve the lead for a later run. The first means the evidence may warrant a change but applying it is unsafe without a decision or further authority; the second means the documentation runner prevented a reliable review. Neither is evidence that the candidate has been accepted or rejected.

`tests/project/project-memory-source-consumption-reconciler.test.ts` verifies processing of consumed candidates and handoffs, legacy `already_trusted` normalization to `already_covered`, missing-ref reporting, absent-state no-op behavior, and fail-closed behavior for malformed state.
