...
- chooses `create` mode only when no curated Project Memory baseline exists, unless `--recreate` forces create mode;
...
Baseline readiness and latest maintenance health are separate gates. `hasCuratedProjectMemoryBaseline` in `src/project/project-memory-state.ts` treats top-level `status: "curated"` and legacy `status: "degraded"` as an existing baseline, so both select maintenance mode. A degraded or failed latest maintenance result remains visible as nested maintenance health and status attention; it does not force a destructive recreate path. The baseline is only usable when canonical markdown remains readable. `src/status/project-memory-inspector.ts` reports this as `curated_with_degraded_maintenance` and warns that the baseline is usable while the latest maintenance result needs attention. `tests/project/project-memory-curator-service.test.ts`, `tests/project/project-memory-packet.test.ts`, and `tests/status/status-service.test.ts` cover the degraded-baseline cases.
...
Create and maintenance packets can cite the sanitized `repository-identity.json` checkout evidence. During canonical publication, `promoteDraftWiki()` rewrites a run-local identity link to `state/<key>/repository-identity.json`; it rejects agent-workspace links and converts target-repository file links into `repo:` citations. Published pages therefore do not depend on temporary authoring paths.
