...
An ingest-created candidate is an evidence-bearing lead, not a free-form note. `src/ingest/worker-output.schema.json` and `parseIngestWorkerOutput` in `src/ingest/worker.ts` require every candidate to provide:

- `evidence.observed_facts` with at least one observed fact, `relevant_paths`, and `uncertainties`;
- `proposed_payload.durable_facts` with at least one proposed fact, `change_kind`, `suggested_subjects`, and `verification_needed`.

The worker prompt requires a `scope: "project"` candidate when leased evidence establishes an implemented durable change to this repository's architecture, public contract, operator workflow, persistence, or layout. It must not record that change only as Session Memory. Creating any project candidate then attempts to schedule detached Project Memory maintenance with the `session_memory_candidate_created` trigger; a scheduling failure does not roll back the durable ingest output. `tests/ingest/worker.test.ts` verifies both the required lead fields and the scheduling call.
