...
`IngestService.start` reports `queued_count` from the raw `experience_events` rows visible when the command begins and derives its selected batch count from that same snapshot. Claimability is determined later by leasing, so a concurrent worker can make a raw queued-row count larger than immediately claimable work. This is an accounting boundary, not a degraded Project Memory condition: auto-maintenance's running-job and lock checks prevent duplicate automatic scheduling, while `SessionStart` remains the explicit bounded-drain exception described below.
