...
## Provider Candidate Contract and Legacy Leads

The evidence-bearing candidate shape belongs to the ingest provider-output boundary, not to Project Memory maintenance input validation. `src/ingest/worker-output.schema.json` and `parseIngestWorkerOutput()` in `src/ingest/worker.ts` require provider-created candidates to include `evidence.observed_facts`, `evidence.relevant_paths`, and `evidence.uncertainties`, plus `proposed_payload.durable_facts`, `change_kind`, `suggested_subjects`, and `verification_needed`. `observed_facts` and `durable_facts` must each contain at least one string; the other four fields are required arrays that may be empty. The worker prompt requires a project-scoped candidate for a verified durable repository change rather than leaving that change only in Session Memory, and `tests/ingest/worker.test.ts` rejects an empty provider evidence object.

Maintenance deliberately accepts older candidates, runtime-inbox candidates, and handoffs whose stored `evidence` or `proposed_payload` is empty. Their metadata is routing context only. The maintenance agent must verify concrete claims against the target repository, regression-test source, and existing canonical documentation before choosing a disposition; it does not reject an otherwise verifiable legacy lead merely because the stored shape predates the provider-output contract. This keeps provider-output validation strict while preserving the review boundary for stored leads.
