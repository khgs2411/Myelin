...
A usable curated baseline remains maintenance-eligible when the latest maintenance result is degraded. `hasCuratedProjectMemoryBaseline()` accepts both `curated` and legacy `degraded` state, and a later degraded maintenance result records `maintenance.status: degraded` without downgrading the top-level canonical baseline from `curated`. `tests/project/project-memory-packet.test.ts` and `tests/project/project-memory-curator-service.test.ts` cover this separation between publication readiness and the latest maintenance health.
...
Maintenance progress is foreground operator feedback, not a separate decision system. `runProjectMemoryMaintenanceMode()` emits started, retry, and terminal progress around the single authoring pass; the bounded retry limit is one retry after the initial attempt. It validates that every applied output reference is a touched path and that every touched `draft-wiki/` path is attributed to an applied source. These checks make source traceability and bounded recovery observable without treating candidate metadata as acceptance proof.
...
For an already-curated project, `myelin memory maintain project <key> --review` creates a review checkpoint. An operator can then use `--promote <run>` to publish that checkpoint without invoking the authoring agent again. `verifyProjectMemoryReviewCheckpoint()` rejects promotion if the reviewed draft or report, canonical Project Memory baseline, target repository snapshot, or reviewed pending-source set changed after review. A successful promotion copies the exact reviewed markdown bytes into a new journaled apply run and records `curation_kind: human_reviewed` plus the reviewed source run. `tests/project/project-memory-curator-service.test.ts` covers exact-byte promotion and rejection on reviewed-artifact or canonical-baseline drift.
