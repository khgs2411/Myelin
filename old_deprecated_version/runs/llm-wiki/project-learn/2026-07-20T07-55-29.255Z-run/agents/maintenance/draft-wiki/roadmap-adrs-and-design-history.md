...
`docs/CLI.md` still gives `myelin bootstrap wizepal --repo /Users/liadgoren/Wizepal/droplet-bot` as its bootstrap example, but that example is stale for Step 12: `docs/ROADMAP.md` requires Droplet Bot to use a distinct Myelin project identity rather than the existing `wizepal` SQLite continuity. The roadmap establishes the boundary, but it does not name the clean-cohort key; select that key before the public dogfood run and update the CLI example and roadmap together.
