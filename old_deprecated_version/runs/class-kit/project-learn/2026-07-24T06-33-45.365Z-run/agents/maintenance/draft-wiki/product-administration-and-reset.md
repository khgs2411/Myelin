...
`apps/class-kit-admin` is the deployed control panel. `src/App.tsx` presents product overview/settings, roles and permissions, users, requests, deployments, and global integrations; its current manifest pins the remote `v0.1.20` SDK tag. The root SDK source switcher can select the local sibling SDK for development or a released remote tag for deployment, and the Pages workflow rejects local or unpinned SDK dependencies. The current published SDK/API baseline is `v0.1.21`, so the manifest needs an explicit update before the panel consumes that release. Its reset panel uses the released `client.admin.products.truncate` when present and retains a direct Edge Function fallback for the same `truncate_product` action.
...
`apps/demo2` is a Vite dogfood/product example, not a product source: its current manifest pins the remote `v0.1.20` SDK tag and consumes the SDK's auth/product behavior. Operators may switch it to the sibling SDK for local dogfooding, and must use an exact remote release tag before deployment. The current published SDK/API baseline is `v0.1.21`, so the manifest needs an explicit update before Demo2 consumes that release. `docs/repositories/structure.md` explicitly classifies both apps as local frontend examples/control surfaces outside the ClassKit product boundary.
