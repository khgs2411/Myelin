...
The current API and SDK package manifests both use `0.1.21`, and canonical integration documentation identifies `v0.1.21` as the published SDK tag. The two app manifests currently remain on remote `v0.1.20`; they are remote-pinned rather than in local dogfood mode, but they do not yet consume the current release.

## Shared SDK source switching

The root `Makefile` exposes `make sdk-local`, `make sdk-remote`, `make sdk-status`, and `make sdk-check-remote` for Demo2 and ClassKit Admin together. `sdk-local` selects `file:../../class-kit-sdk` in both app manifests for local dogfooding. `sdk-remote` finds the latest stable `vX.Y.Z` tag in the SDK checkout, pins both manifests to the explicit Git SSH release URL, updates both lockfile ecosystems, and builds the apps workspace.
...
`apps/` is an npm workspace containing `class-kit-admin` and `class-kit-demo2`. Both current manifests select the remote `v0.1.20` SDK tag, not `file:../../class-kit-sdk`; that is a valid deployable source mode, but it remains behind the current `v0.1.21` release. Local dogfooding requires `make sdk-local`. Operators must use `make sdk-remote` and `make sdk-check-remote` before deploying or distributing an application, then confirm the selected tag is the intended release.
