...
The `apps/` workspace can use a sibling SDK during active local iteration, while deployed consumers should pin a released SDK tag over Git SSH. Both checked-in local apps currently depend on the SDK tag `v0.1.18`, consistent with that consumer role. An app may administer or demonstrate ClassKit, but it does not own migrations, Edge Functions, or the canonical product API.
