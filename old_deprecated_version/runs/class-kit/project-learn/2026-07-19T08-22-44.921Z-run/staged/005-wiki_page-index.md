...
The authored root documentation calls the parent repository local-only with no remote. That claim conflicts with the sanitized checkout evidence inspected for this maintenance pass. Checkout identity is run evidence, not a durable navigation fact; treat the authored no-remote claim as stale or needing repair rather than as an authoritative repository boundary.
