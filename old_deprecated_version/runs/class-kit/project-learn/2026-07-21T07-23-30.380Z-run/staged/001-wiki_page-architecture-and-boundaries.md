...
The supplied `class-kit-sdk` source has no typed customer-management facade for `class-kit-customers`. The Edge Function is therefore an implemented backend/manager contract, but product browser applications must not bypass the SDK boundary by calling it raw while a typed consumer surface is absent.

These claims are grounded in `20260720085642_customer_identity_foundation.sql`, `20260720090353_customer_crud_and_access_separation.sql`, the customer-oriented membership/registration/attendance/discovery migrations, and the `class-kit-customers`, shared-context, membership, registration, and attendance handlers. Focused handler tests cover customer CRUD shape/lifecycle, canonical versus legacy membership and walk-in targeting, direct ghost registration, and inactive-customer error mapping. The snapshot has no focused database regression suite that exercises the full ghost lifecycle, cross-product nondisclosure, or deactivation under concurrent membership/registration mutations, and no typed SDK facade yet proves the customer API is consumable by product browsers.
...
- The supplied root checkout excludes the ignored `class-kit-api/` and `class-kit-sdk/` working trees, so this maintenance run cannot independently re-read the new repository-local guides or run their native checks. The guidance above is limited to the pending source's observed file and workflow facts.
