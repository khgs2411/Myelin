...
Memberships are product-scoped manager-issued grants attached to a customer. They determine whether that customer is entitled to member-only or membership-required classes and, for stock-bearing grants, how many approved registrations remain.

## Customer recipient boundary

Membership grants and their ledger use `customer_id` as the canonical service-recipient identity. A manager action may target either a canonical `customer_id` or a legacy linked `user_id`, but never both: dual identifiers are rejected, and the legacy path resolves only a linked customer in the current product. That makes ghost customers eligible for the same membership lifecycle without granting them an access role or self-service capability.

For customer-targeted `grant`, `set_for_customer`, and `upgrade` operations, the server first applies ordinary origin/product and manager permission checks, then requires an active customer in that product before it validates the membership type and applies grant/stock rules. An inactive customer therefore receives `409 customer_inactive` and gets no new grant or ledger entry. Revoke/history operations retain the customer target so historical entitlement evidence is not deleted. `class-kit-memberships/index.test.ts` covers canonical RPC selection, legacy linked-user wrappers, dual-ID rejection, and the inactive-customer error envelope; the migration `20260720091436_customer_membership_entrypoints.sql` enforces the product/customer relationship in SQL.
