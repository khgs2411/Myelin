...
Both password signup and platform-admin user creation call Supabase Auth's `auth.admin.createUser`; neither path pre-lists Auth users to detect a duplicate email. The shared helper translates Auth's recognized duplicate-email responses (`email_exists`, or a 409/422 duplicate-email message) into `409 conflict` with `A user with this email already exists.` Other Auth create failures become `500 internal_error`. Successful Auth creation is followed by profile persistence when a display name was supplied and then product-role assignment; a later profile or assignment failure returns `500` even though the Auth user has already been created (`class-kit-api/supabase/functions/_shared/auth_users.ts`, `class-kit-product-signup/index.ts`, `class-kit-admin-product-users/index.ts`).
...
Implementation evidence is the shared context and permission helpers, product-context/profile/registration Edge Functions, and the current migration chain named above. The available SQL regressions cover member auto-approval, pending-registration cancellation, schedule backfill, and destructive product reset; the reset regression incidentally preserves origin, access-entry, role, and redirect records. They do not directly exercise origin matching, local product-key ambiguity, provider denial, every access-state branch, permission-scope fallback, final role-management actor protection, or the Auth duplicate-email/error-mapping contract. The historical permission-layer smoke checklist describes those scenarios, but it is design guidance rather than current regression proof; these contracts therefore retain that coverage gap.
