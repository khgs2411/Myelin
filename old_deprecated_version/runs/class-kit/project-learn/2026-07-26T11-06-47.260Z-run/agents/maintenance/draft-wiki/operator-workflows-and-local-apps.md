...
Command ownership follows the independently versioned repositories described in `docs/repositories/structure.md`; the root Makefile also coordinates the shared SDK source mode for its two app consumers:
...
The current API and SDK package manifests and both app manifests use `0.1.23`. The apps are remote-pinned rather than in local dogfood mode, so they consume the published release rather than the sibling SDK.

## Shared SDK source switching

The root `Makefile` exposes `make sdk-local`, `make sdk-remote`, `make sdk-status`, and `make sdk-check-remote` for Demo2 and ClassKit Admin together. `sdk-local` selects `file:../../class-kit-sdk` in both app manifests for local dogfooding. `sdk-remote` reads the SDK version from `class-kit-sdk/package.json`, verifies that exact `vX.Y.Z` tag exists in the Git SSH repository, pins both manifests to that release URL, refreshes the apps workspace with `npm update @class-kit/react --workspaces`, regenerates the standalone Admin Bun lockfile with an isolated cache, and builds the workspace. It does not select a release by semver-sorting remote tags.

The switcher snapshots both manifests and lockfiles first. If source replacement, dependency installation, or the app build fails, it restores those snapshots and attempts dependency restoration; this keeps the source-mode change rollback-safe even when the environment still needs a later reinstall. `sdk-check-remote` rejects either app unless its dependency is an exact remote semantic-version tag. The ClassKit Admin GitHub Pages workflow independently runs the same exact-tag check before installing dependencies, so a local checkout or unpinned remote source cannot be deployed through that workflow.

## Administrative and dogfood consumers

`apps/` is an npm workspace containing `class-kit-admin` and `class-kit-demo2`. Both current manifests select the remote `v0.1.23` SDK tag, not `file:../../class-kit-sdk`; this is the current deployable source mode. Local dogfooding requires `make sdk-local`. Operators must use `make sdk-remote` and `make sdk-check-remote` before deploying or distributing an application, then confirm the selected tag is the intended release.
...
The app invokes SDK admin surfaces for product creation, auth policy and redirect management, origin management, users and roles, reset, change requests, and PM integrations. Those UI actions are requests to the backend authorization contract; the Vite app does not itself grant authority or mutate database tables directly. It stores its own auth state under `class-kit-admin-auth`, defaults to the remote target for production builds, and can point to local Supabase while developing. Its Vite server is configured for port `5175`; its GitHub Pages workflow requires an exact remote SDK tag before it builds, preserving the deployed/control-panel boundary despite the current local development manifest.
...
- This snapshot contains the ClassKit Admin GitHub Pages workflow but does not establish a successful current hosted build or deployment. Credentials, Pages configuration, and deploy-key access remain operator verification points.
