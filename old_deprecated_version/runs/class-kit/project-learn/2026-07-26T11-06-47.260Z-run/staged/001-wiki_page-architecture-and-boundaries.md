...
The `apps/` workspace can use a sibling SDK during active local iteration, while deployed consumers must pin a released SDK tag over Git SSH. Both `class-kit-admin` and Demo2 currently pin the remote `v0.1.23` tag, which matches the current released SDK/API baseline. The root source switcher can select the sibling path for local dogfooding or replace both pins with the exact tag derived from `class-kit-sdk/package.json` after verifying it exists remotely; the Admin deployment workflow rejects local or unpinned dependencies. An app may administer or demonstrate ClassKit, but it does not own migrations, Edge Functions, or the canonical product API.
