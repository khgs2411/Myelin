...
`apps/class-kit-admin` is the deployed control panel. `src/App.tsx` presents product overview/settings, roles and permissions, users, requests, deployments, and global integrations; its current manifest pins the remote `v0.1.23` SDK tag, matching the current published SDK/API baseline. The root SDK source switcher can select the local sibling SDK for development or the exact released tag derived from the SDK package manifest for deployment, and the Pages workflow rejects local or unpinned SDK dependencies. Its reset panel uses the released `client.admin.products.truncate` when present and retains a direct Edge Function fallback for the same `truncate_product` action.
...
`apps/demo2` is a Vite dogfood/product example, not a product source: its current manifest pins the remote `v0.1.23` SDK tag and consumes the SDK's auth/product behavior. Operators may switch it to the sibling SDK for local dogfooding, and must use an exact remote release tag before deployment. `docs/repositories/structure.md` explicitly classifies both apps as local frontend examples/control surfaces outside the ClassKit product boundary.
