...
`create_user` creates an Auth user and profile, then assigns it to the named product as `manager` or `user` (default `user`) with an active role assignment. If membership assignment fails after Auth-user creation, it returns an error but does not roll back the Auth user; this is an operational recovery case.
