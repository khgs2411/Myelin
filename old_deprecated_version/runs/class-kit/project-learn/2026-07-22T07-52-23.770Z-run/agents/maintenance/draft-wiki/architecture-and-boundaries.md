...
The SDK exposes `management.customers.list`, `get`, `create`, `update`, `deactivate`, and `reactivate`. Its customer shape uses `customerId`, `userId`, `identityStatus`, `customerOrigin`, and `status`; registration summaries likewise carry `customerId`, nullable `userId`, and customer status. It deliberately does not expose the backend merge actions, so a product browser must not call `class-kit-customers` raw to merge customers.
