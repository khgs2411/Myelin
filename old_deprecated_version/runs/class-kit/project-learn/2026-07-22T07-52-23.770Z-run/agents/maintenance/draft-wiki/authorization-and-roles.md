...
| Memberships | Level 75 for operational lists; `memberships.manage` for `create_type`, `update_type`, `deactivate_type`, `grant`, `set_for_customer` (and linked-user-compatible `set_for_user`), `upgrade`, and `revoke`; `memberships.adjust_stock` for `adjust_stock` in `class-kit-memberships`. | Types support `stock`, `limited_stock`, `limited`, and `infinite` modes. Authorized actors can create and deactivate types, alter grants, revoke entitlement, and alter remaining stock. Revocation/deactivation can remove availability; stock adjustment changes future registration capacity. |
| Customers | Product level 75 for `list`, `get`, `create`, `update`, `deactivate`, and `reactivate` in `class-kit-customers`. | Managers control product-owned linked and ghost customer records, but cannot change identity, role, or access fields through this resource. Deactivation preserves history and blocks new customer-targeted membership, registration, and walk-in mutations until reactivation. |
