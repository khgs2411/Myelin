...
- `customers`: list/get/create/update/deactivate/reactivate product customers. Customer records expose a stable customer ID, nullable linked `userId`, identity and origin summaries, and `active`/`inactive` status. Registration summaries include the same `customerId`, `userId`, and customer-status projection so managers can distinguish ghost and linked registrations;
