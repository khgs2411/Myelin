...
`apps/` is an npm workspace containing `class-kit-admin` and `class-kit-demo2`. Both current manifests select `file:../../class-kit-sdk` for local iteration, so their build input is the sibling checkout rather than a published tag. That is a local dogfood convenience only: operators must use `make sdk-remote` and `make sdk-check-remote` before deploying or distributing an application.
