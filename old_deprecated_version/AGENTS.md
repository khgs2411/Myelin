...
| `INGEST_LLM_TIMEOUT_MS` | Per-turn curator provider timeout. This repository uses 20 minutes because complete proposal turns must close every active-memory disposition and can exceed the 10-minute runtime default. |
...
- Proposal agents classify sources and cite source IDs on staged outputs and memory dispositions. The trusted coordinator derives the redundant canonical `output_refs` closure and `checked_output_refs`; provider-authored reference spellings are not an authority boundary.
...
- Codex hooks distinguish root tasks from headless/internal agent sessions using the hook transcript identity. Headless `SessionStart` and `UserPromptSubmit` payloads are orchestration noise and are not persisted or scheduled; a concise headless `Stop` result remains eligible evidence. Historical headless prompts and any item above the absolute evidence-item limit are finalized as reviewable no-output tombstones with their complete raw evidence retained, so one non-curatable item cannot block the rest of the queue.
- A Myelin runtime must refuse a memory database whose schema is newer than the runtime supports. Auto-maintenance reports `installed_runtime_incompatible` without launching a detached worker; activate the matching installed runtime before expecting hooks to resume maintenance.
