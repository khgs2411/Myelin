...
export function registerSessionCommands(cli: Cli): void {
  cli.command(["session", "start"], (args) => start(args));
  cli.command(["session", "log"], (args) => log(args));
  cli.command(["session", "close"], (args) => close(args));
  cli.command(["session", "recent"], (args) => recent(args));
  cli.command(["session", "show"], (args) => show(args));
}
...
async function start(args: string[]): Promise<CommandResult> {
  ...
  try {
    const result = await service().start(p.projectKey, p.title);
...
async function log(args: string[]): Promise<CommandResult> {
  ...
  try {
    const result = await service().log({ projectKey: p.projectKey, message: p.message, kind: p.kind, sessionId: p.session });
...
async function close(args: string[]): Promise<CommandResult> {
  ...
  try {
    const result = await service().close({ projectKey: p.projectKey, summary: p.summary, sessionId: p.session });
...
async function recent(args: string[]): Promise<CommandResult> {
  const p = parse(args, { limit: true });
  if (p.error) return fail(p.error);
  if (!p.projectKey) return fail("Usage: myelin session recent <key> [--limit N] [--json]");
  try {
    const result = await service().recent(p.projectKey, p.limit ?? 5);
...
async function show(args: string[]): Promise<CommandResult> {
  const p = parse(args, {});
  if (p.error) return fail(p.error);
  if (!p.projectKey) return fail("Usage: myelin session show <session-id> [--json]");
  try {
    const found = await service().show(p.projectKey); // first positional is the session id here
...
function service(): SessionService {
  return new SessionService(repoRoot().root);
}
