...
export {
  ...
  listSessionMemoryAnchorJobs,
  readIngestJobWithAnchor,
  transitionSessionMemoryAnchorJob,
} from "../session-maintenance/job-lifecycle.ts";
export type {
  SessionMemoryAnchorJobCasFailureCode,
  SessionMemoryAnchorJobCasResult,
} from "../session-maintenance/job-lifecycle.ts";
import {
  isLegacySessionJobOwnerDenied,
  withAnchorPrepareAdmission,
  withCompatibilityJobCreateAdmission,
  withCompatibilityJobTransitionAdmission,
} from "../memory/session-memory-write-firewall.ts";
...
export function createIngestJob(db: Database, input: CreateIngestJobInput): IngestJobRow {
  withCompatibilityJobCreateAdmission(db, input.project_key, () => {
    db.query(
...
export function createAnchorIngestJobInOpenTransaction(
  db: Database,
  input: CreateIngestJobInput & { owner_epoch: number },
): IngestJobRow {
  if (!db.inTransaction) throw new Error("SMC anchor job creation requires an open transaction");
  if (isLegacySessionJobOwnerDenied(db, input.id)) {
    throw new Error(`session_memory_anchor_legacy_denied: ${input.id}`);
  }
...
export function updateIngestJobStatus(db: Database, input: UpdateIngestJobStatusInput): IngestJobRow {
  ...
  withCompatibilityJobTransitionAdmission(db, existing.project_key, existing.id, () => {
    db.query(
      `UPDATE ingest_jobs
     SET status = ?,
         provider_session_id = ?,
