...
export function renderLauncher(locatorPath: string): string {
  return [
    ...
    '  if (locator?.schema_version !== 1 || !isAbsolute(locator?.myelin_root ?? "") || !isAbsolute(locator?.launcher?.path ?? "")) throw new Error("invalid locator");',
    "  const root = locator.myelin_root;",
    '  if (!statSync(root).isDirectory()) throw new Error(`recorded Myelin root is missing: ${root}`);',
    ...
    '  const child = Bun.spawnSync([process.execPath, join(root, "src", "cli.ts"), ...process.argv.slice(2)], {',
    "    cwd: process.cwd(),",
    "    env: process.env,",
