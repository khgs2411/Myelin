...
- Launch one or more detached provider sessions from the target repo, with Myelin-owned job and pull/tombstone state.

**Consequences**

The first implementation runs one detached ingest agent by default and designs atomic pull claims so multi-agent partitioning can be added later without changing the public ingest model. Myelin should optimize prompts, tools, and durable state rather than prescribe the number or shape of memory records created from each batch.
