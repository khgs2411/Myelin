...
>
> Inherited baseline copied from the application-wide design. Its flat
> `WorkspaceContext` and `resolveProjectRoot` surface require reconciliation
> with the unified unit. Use it as review evidence only until the workspace
> resolution issue is resolved and this artifact is revised.
>
...
that reference as part of the normalized observation. `EvidenceCaptureService`
combines the observation and `WorkspaceContext` when it constructs evidence.
