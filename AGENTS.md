...
- Enums use the matching `EEnumName` convention.
- Constants use `ALL_CAPS_WITH_UNDERSCORES`.
- Static method names use `UpperCamelCase`.
- Declare `public` or `private` on every class member. Do not use TypeScript's implied `public`
  access.
