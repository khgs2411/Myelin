...
- Optional auto Session Memory maintenance is scheduled from capture hooks only after the Experience Log row is stored. Ordinary ingest uses the configured threshold and cooldown; `SessionStart` forces a bounded drain of leftover rows. Newly created Session Memory also schedules its derived index independently of the capture threshold. Hooks do not run ingest or indexing synchronously. Codex `Stop` means assistant turn complete, not session end.
