...
test("maintenance curator output schema imports operation and lifecycle enums", () => {
  ...
  expect(item.properties.target_page.properties.path_kind.enum).toEqual(["existing_wiki_page"]);
  expect(item.required).toContain("apply_payload");
  expect(item.properties.target_entry_id.anyOf).toContainEqual({ type: "null" });
  expect(item.properties.proposed_entry_id.anyOf).toContainEqual({ type: "null" });
});
