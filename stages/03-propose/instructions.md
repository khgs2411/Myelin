...
- `acceptance_questions` - the lookups a cold future Claude session will run against the wiki (from `projects/<key>/acceptance-questions.md`, generated by stage 05-acceptance); may be empty on first run
...
### Using `acceptance_questions`

Each entry has: `index`, `tag` (`discipline|lookup|relationship|runbook|decision`), `text`, and optional `targets` (the ranked domain) and `expected` (the wiki page that should answer it).

Treat acceptance questions as a **content-coverage contract** on your proposal:

- For every question with a `targets` domain, the page for that domain (the one you're creating or updating) MUST contain enough concrete detail to answer the question from the page text alone - specific mechanisms, reducer/function names, handoff points, file:line citations - not just the conceptual "what."
- A page that describes *that* something happens but not *how* it happens will score 1/2 on measure. Prefer naming the mechanism: which reducer fires, what writes back to which table, which function is the handoff point, which flag / status / enum value signals the transition.
- If a question cannot be answered from sources (spec and code don't document the mechanism), it is acceptable for the page to still score 1/2 - but surface that gap honestly as an `## Open Questions` bullet with a source-reading follow-up, rather than silently omitting the topic.
- If a question's `targets` domain is NOT in the ranking snapshot, that's a signal the ranking missed a real lookup - either promote that domain into a unit or flag it in `deferred_domains` with a clear reason.

This is a proposal-authoring constraint, not a hard validator (validate does not block on measure scores). But a well-authored proposal will produce pages that answer every question, and next cycle's measure run will confirm it.
...
- `index.md` MAY include a final `## Status` block pointing at machine-readable state files (freshness.json, ranking-snapshot.json, etc.) - this is the one place where pointing at wiki infrastructure is allowed.
